...
public class HexRGrabbable : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        if ( RHandParent != null) { RfingerUseTracking = RightHand.GetComponent<FingerUseTracking>(); }
        else { Debug.Log("Right hand is not found"); }

        if (LHandParent != null) { LfingeruseTracking = LeftHand.GetComponent<FingerUseTracking>(); }
    ...
    private void IsGrab(GameObject HandParent, FingerUseTracking fingerUseTracking)
    {
        ...
        {
            // Invoke the event when a trigger occurs
            OnGrab?.Invoke();
        }
